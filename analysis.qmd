...
In @fig-line-girls, you can see peaks in popularity for names such as Lisa, Jennifer and Jessica.

Here we visualize the top 10 most frequent boy names from 1950 onwards:
...
@fig-line-boys shows that several names that were popular before 1990 became relatively less frequent after 2000.

## Girl name trends from 2010 onwards

The two line plots in @fig-line-2010-1 and @fig-line-2010-2 show the popularity trends of the most frequent girl names from 2010 onwards. To compare a larger number of names simultaneously, @fig-line-2010-3 presents a heatmap of the top 30 names.
...
most_freq_girls <- get_most_frequent(
  babynames,
  select_sex = "F",
  from = from_year
)
...
prop_df <- babynames |>
  filter(
    name %in% most_freq_girls$most_frequent$name[1:30],
    sex == "F"
  ) |>
